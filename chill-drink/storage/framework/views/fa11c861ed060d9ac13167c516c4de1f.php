...
<head>
    ...
    <title>Quản trị cấp cao - <?php echo e(config('app.name', 'Chill Drink')); ?></title>
...
<body>
    <div class="root-shell">
        <aside class="root-sidebar" data-root-sidebar>
            <a href="<?php echo e(route('admin.super-admin')); ?>" class="root-brand">
                <img src="<?php echo e(asset('images/logo.png')); ?>" alt="Chill Drink" class="root-brand-mark">
            ...
            <nav class="root-nav">
                ...
                <a href="<?php echo e(route('admin.super-admin')); ?>" class="root-nav-link" data-root-section="top"><i class="bi bi-grid-1x2"></i> Tổng quan</a>
                <a href="<?php echo e(route('admin.super-admin') . '#admins'); ?>" class="root-nav-link" data-root-section="admins"><i class="bi bi-person-badge"></i> Quản trị viên <span class="root-nav-badge"><?php echo e(\App\Models\User::admins()->count()); ?></span></a>
                <a href="<?php echo e(route('admin.super-admin') . '#branch-ranking'); ?>" class="root-nav-link" data-root-section="branch-ranking"><i class="bi bi-shop"></i> Chi nhánh <span class="root-nav-badge"><?php echo e(\App\Models\Branch::count()); ?></span></a>
                <a href="<?php echo e(route('admin.super-admin') . '#permissions'); ?>" class="root-nav-link" data-root-section="permissions"><i class="bi bi-key"></i> Vai trò và phân quyền</a>
                <a href="<?php echo e(route('admin.chat.index')); ?>" class="root-nav-link <?php echo e(request()->routeIs('admin.chat.*') ? 'active' : ''); ?>">
                    ...
                    <?php if($unreadChatMessages > 0): ?>
                        <span class="badge rounded-pill bg-danger ms-auto" style="font-size: 0.72rem;"><?php echo e($unreadChatMessages > 99 ? '99+' : $unreadChatMessages); ?></span>
                    <?php endif; ?>
                </a>
                ...
                <a href="<?php echo e(route('admin.orders.index')); ?>" class="root-nav-link <?php echo e(request()->routeIs('admin.orders.*') ? 'active' : ''); ?>"><i class="bi bi-receipt"></i> Đơn hàng</a>
                <a href="<?php echo e(route('admin.group-orders.index')); ?>" class="root-nav-link <?php echo e(request()->routeIs('admin.group-orders.*') ? 'active' : ''); ?>"><i class="bi bi-people-fill"></i> Đơn nhóm</a>
                <a href="<?php echo e(route('admin.reviews.index')); ?>" class="root-nav-link <?php echo e(request()->routeIs('admin.reviews.*') ? 'active' : ''); ?>"><i class="bi bi-chat-square-text"></i> Đánh giá</a>
                <a href="<?php echo e(route('admin.users.index')); ?>" class="root-nav-link <?php echo e(request()->routeIs('admin.users.*') ? 'active' : ''); ?>"><i class="bi bi-people"></i> Khách hàng</a>

                <p class="root-nav-label">Hệ thống</p>
                <a href="<?php echo e(route('admin.super-admin')); ?>#security" class="root-nav-link" data-root-section="security"><i class="bi bi-shield-check"></i> Bảo mật</a>
                <a href="<?php echo e(route('admin.super-admin')); ?>#health" class="root-nav-link" data-root-section="health"><i class="bi bi-activity"></i> Tình trạng hệ thống</a>
                <a href="<?php echo e(route('admin.super-admin')); ?>#audit" class="root-nav-link" data-root-section="audit"><i class="bi bi-journal-text"></i> Nhật ký hệ thống</a>
            </nav>

            <div class="root-sidebar-footer">
                <a href="<?php echo e(route('home')); ?>" class="root-nav-link"><i class="bi bi-house-door"></i> Về trang chủ</a>
            </div>
        </aside>
        ...
        <div class="root-content">
            <header class="root-topbar">
                <div class="root-topbar-left">
                    ...
                    <div class="root-breadcrumb"><span>Chill Drink / Hệ thống / </span><strong><?php echo $__env->yieldContent('page-title', 'Quản trị cấp cao'); ?></strong></div>
                </div>
                <div class="root-topbar-right">
                    <form class="root-topbar-search" method="GET" action="<?php echo e(route('admin.super-admin')); ?>" role="search">
                        <i class="bi bi-search"></i>
                        <input type="search" name="q" value="<?php echo e(request('q')); ?>" placeholder="Tìm tên hoặc email Admin" aria-label="Tìm quản trị viên">
                    </form>
                    <div class="dropdown">
                        <button type="button" class="root-topbar-btn position-relative" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Thông báo">
                            ...
                            <?php if(($unreadNotificationCount ?? 0) > 0): ?>
                                <span class="root-notification-count"><?php echo e(min(99, $unreadNotificationCount)); ?></span>
                            <?php endif; ?>
                        </button>
                        <div class="dropdown-menu dropdown-menu-end root-dropdown">
                            <div class="root-dropdown-title">Thông báo</div>
                            <?php $__empty_1 = true; $__currentLoopData = ($notifications ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <div class="dropdown-item-text root-notification-item">
                                    <strong><?php echo e(data_get($notification->data, 'title', 'Thông báo hệ thống')); ?></strong>
                                    <span><?php echo e(data_get($notification->data, 'message', $notification->created_at?->diffForHumans())); ?></span>
                                </div>
                        ...
                        <button type="button" class="root-user-button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Tài khoản quản trị cấp cao">
                            <span class="root-user-avatar">
                                <?php if(Auth::user()->avatar): ?>
                                    <img src="<?php echo e(str_starts_with(Auth::user()->avatar, 'http') ? Auth::user()->avatar : asset('storage/'.ltrim(Auth::user()->avatar, '/'))); ?>" alt="<?php echo e(Auth::user()->name); ?>">
                                <?php else: ?>
                                    <?php echo e(mb_strtoupper(mb_substr(Auth::user()->name, 0, 1))); ?>

                                <?php endif; ?>
                            </span>
                            <span class="root-user-copy"><strong><?php echo e(Auth::user()->name); ?></strong><span>Quản trị cấp cao</span></span>
                            <i class="bi bi-chevron-down small"></i>
                        </button>
                        <div class="dropdown-menu dropdown-menu-end root-dropdown">
                            <a class="dropdown-item" href="<?php echo e(route('profile.edit')); ?>"><i class="bi bi-person me-2"></i>Thông tin cá nhân</a>
            ...
            <footer class="root-footer">
                ...
                <span>Phiên bản 1.0.0 · Cập nhật <?php echo e(now()->format('d/m/Y')); ?></span>
            </footer>
        </div>
    </div>
        ...
        const updateRootNavigation = () => {
            ...
            document.querySelectorAll('.root-nav-link').forEach((link) => {
                ...
                if (section) {
                    ...
                    const isSuperAdminPage = currentPath === '<?php echo e(parse_url(route("admin.super-admin"), PHP_URL_PATH)); ?>';
...
<?php /**PATH C:\xampp\htdocs\DATN\chill-drink\resources\views/layouts/super-admin.blade.php ENDPATH**/ ?>