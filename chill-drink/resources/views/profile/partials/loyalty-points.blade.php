        ...
        <div class="row align-items-center">
            <div class="col-lg-8">
                <div class="mb-3">
                    ...
                    <div class="points-display">{{ number_format($loyaltyPoint->total_points, 0, ',', '.') }}</div>
            ...
            <div class="col-lg-4 text-lg-end mt-4 mt-lg-0">
                <div class="d-flex flex-column gap-3">
                    <div class="bg-white bg-opacity-25 backdrop-blur rounded-3 p-3 shadow-sm">
                        ...
                        <div class="h4 fw-bold text-white mb-0">{{ number_format($loyaltyPoint->monthly_points, 0, ',', '.') }}</div>
                    </div>
                    <div class="bg-white bg-opacity-25 backdrop-blur rounded-3 p-3 shadow-sm">
                        <div class="text-white small mb-1" style="opacity: 0.9;">Tổng điểm tích lũy</div>
                        <div class="h4 fw-bold text-white mb-0">{{ number_format($loyaltyPoint->lifetime_points, 0, ',', '.') }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="tab-pane fade show active" id="redeemVouchers">
            <div class="row g-4">
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="voucher-redeem-card {{ !$canRedeem ? 'disabled' : '' }}" 
                         onclick="{{ $canRedeem ? "document.getElementById('redeem-form-{$voucher->id}').submit()" : '' }}">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div class="point-cost-badge">
                                <i class="bi bi-coin me-1"></i>{{ number_format($voucher->point_cost, 0, ',', '.') }} điểm
                            </div>
                        </div>

                        <div class="mb-3">
                            <h5 class="fw-bold mb-2">{{ $voucher->code }}</h5>
                            <p class="text-secondary small mb-2">{{ $voucher->description }}</p>
                            <div class="fw-bold text-primary">
                                Giảm {{ $voucher->type === 'percent' ? $voucher->value . '%' : number_format($voucher->value, 0, ',', '.') . 'đ' }}
                                @if($voucher->max_discount)
                                <span class="text-secondary small">(Tối đa {{ number_format($voucher->max_discount, 0, ',', '.') }}đ)</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="alert alert-danger mb-0 p-2 small">
                            <i class="bi bi-x-circle me-1"></i>Không đủ điểm (thiếu {{ number_format($voucher->point_cost - $loyaltyPoint->total_points, 0, ',', '.') }} điểm)
                        </div>
                        @else
                        <form id="redeem-form-{{ $voucher->id }}" method="POST" action="{{ route('loyalty.redeem-voucher', $voucher) }}" style="display: none;">
                            @csrf
                        </form>
                        <button type="button" class="btn btn-primary w-100" onclick="event.stopPropagation(); document.getElementById('redeem-form-{{ $voucher->id }}').submit();">
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="d-flex gap-3">
                            <div class="flex-shrink-0">
                                <i class="{{ $transaction->typeIcon() }}" style="font-size: 2rem;"></i>
                            </div>
                            <div>
                                <div class="fw-bold mb-1">{{ $transaction->typeDisplayName() }}</div>
                                <div class="text-secondary small mb-1">{{ $transaction->description ?? 'Không có mô tả' }}</div>
                                <div class="text-muted small">{{ $transaction->created_at->format('d/m/Y H:i') }}</div>
                            </div>
                        </div>
                        <div class="text-end">
                            <div class="fw-bold {{ $transaction->points > 0 ? 'text-success' : 'text-danger' }}" style="font-size: 1.25rem;">
                                {{ $transaction->formattedPoints() }}
                            </div>
                            <div class="text-secondary small">Số dư: {{ number_format($transaction->balance_after, 0, ',', '.') }}</div>
                        </div>
                    </div>
                </div>
                @empty
