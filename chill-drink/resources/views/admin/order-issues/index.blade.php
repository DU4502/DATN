...
<div class="container-fluid py-4 issue-board">
    ...
    @forelse($reports as $issue)
        @php [$statusLabel, $statusColor] = $statuses[$issue->status] ?? ['Đang chờ', 'secondary']; $availableStatuses = $issue->availableAdminStatuses(); @endphp
        <article class="issue-card"><div class="issue-grid">
            ...
            @if($issue->status === 'resolved')
                ...
                <section class="issue-result" aria-label="Kết quả xử lý khiếu nại">
                    ...
                    @elseif($issue->resolution_type === 'redelivery' && $issue->redeliveryOrder)
                        <div class="issue-result-voucher">
                            <div class="issue-help">Đơn giao bù miễn phí cho {{ $issue->order->displayCode() }}</div>
                            <div class="issue-result-code"><i class="bi bi-truck me-1"></i>{{ $issue->redeliveryOrder->displayCode() }}</div>
                            <div class="issue-meta mt-1"><strong>{{ \App\Support\OrderStatus::label((string) $issue->redeliveryOrder->status) }}</strong>@if($issue->redeliveryOrder->status_changed_at) · {{ $issue->redeliveryOrder->status_changed_at->format('d/m/Y H:i') }}@endif</div>
                            <div class="issue-result-note"><strong>Sản phẩm đã giao bù:</strong><br>@forelse($issue->redeliveryOrder->orderItems as $replacementItem){{ $replacementItem->quantity }}× {{ $replacementItem->product?->name ?? 'Sản phẩm' }}@if($replacementItem->sugar_level !== null) · Đường {{ $replacementItem->sugar_level }}%@endif @if($replacementItem->ice_level !== null) · Đá {{ $replacementItem->ice_level }}%@endif @if(!$loop->last)<br>@endif @empty Chưa có thông tin món giao bù.@endforelse</div>
                        </div>
                        @if($issue->resolution_value)<div class="issue-result-note"><strong>Nội dung phương án:</strong> {{ $issue->resolution_value }}</div>@endif
            ...
            @elseif($issue->status === 'awaiting_confirmation' && $issue->redeliveryOrder)
                <section class="issue-result is-progress" aria-label="Tiến độ giao bù">
                    <div class="issue-result-head"><span class="issue-result-icon"><i class="bi bi-truck"></i></span><div><div class="issue-result-title">Đang thực hiện giao bù</div><div class="issue-help">Yêu cầu sẽ tự hoàn tất sau khi khách nhận hàng</div></div></div>
                    <div class="issue-label">Đơn giao bù miễn phí</div>
                    <div class="issue-result-code">{{ $issue->redeliveryOrder->displayCode() }}</div>
                    <div class="issue-meta"><strong>{{ \App\Support\OrderStatus::label((string) $issue->redeliveryOrder->status) }}</strong>@if($issue->estimated_at) · Dự kiến {{ $issue->estimated_at->format('d/m/Y H:i') }}@endif</div>
                    <div class="issue-result-note"><strong>Món giao bù:</strong><br>@foreach($issue->redeliveryOrder->orderItems as $replacementItem){{ $replacementItem->quantity }}× {{ $replacementItem->product?->name ?? 'Sản phẩm' }}@if(!$loop->last)<br>@endif @endforeach</div>
                    @if($issue->resolution_value)<div class="issue-result-note"><strong>Phương án:</strong> {{ $issue->resolution_value }}</div>@endif
                    @if($issue->admin_note)<div class="issue-result-note"><strong>Phản hồi:</strong> {{ $issue->admin_note }}</div>@endif
                    <div class="issue-result-foot"><i class="bi bi-shield-check"></i><span>Đã khóa phương án, không cần lưu lại</span></div>
                </section>
                ...
                <form class="issue-action" method="POST" action="{{ route($issueRoute('update'), $issue) }}" data-order-total="{{ (int) $issue->order->total }}" data-voucher-issued="{{ $issue->voucher_coupon_id ? '1' : '0' }}" data-redelivery-created="{{ $issue->redelivery_order_id ? '1' : '0' }}">
                    ...
                    <div class="issue-form-grid">
                        @if(count($availableStatuses) === 1)
                            ...
                            <select class="form-select js-issue-status" disabled aria-label="Trạng thái đã khóa"><option value="{{ $issue->status }}" selected>{{ $statusLabel }}</option></select>
                        @else
