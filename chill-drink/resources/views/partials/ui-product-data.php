...
if (! view()->shared('ui.product.bootstrapped', false)) {
    ...
    $uiGetProductGallery = static function (?string $sku, ?string $category, ?string $name = null, int $limit = 4) use ($uiProductImageUrls, $uiCategoryImagePools, $uiResolveProductImage, $uiResizeImage): array {
        $images = [];

        if ($sku && isset($uiProductImageUrls[$sku])) {
            $images[] = $uiProductImageUrls[$sku];
        }

        if ($category && isset($uiCategoryImagePools[$category])) {
            foreach ($uiCategoryImagePools[$category] as $url) {
        ...
        if (count($images) === 0) {
            $images[] = $uiResolveProductImage($sku, $category, $name, 1000);
        }

        return array_values(array_map(
            fn (string $url) => $uiResizeImage($url, 400),
