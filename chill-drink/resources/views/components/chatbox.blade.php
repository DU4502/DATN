<style>
    ...
    .chat-unread-badge {
        ...
        background: #dc3545;
        color: #fff;
        box-shadow: 0 4px 10px rgba(220, 53, 69, 0.32);
        font-size: 11px;
        font-weight: 800;
        line-height: 1;
        display: flex;
...
<div
            ...
            this.echoChannel = window.Echo.private('conversation.' + this.conversationId)
                ...
                .listen('.conversation-closed', (payload) => {
                    ...
                    if (this.isOpen) {
                        ...
                        this.getOrCreateConversation().then(() => this.requestLocationAndFetchBranches());
                    }
                })
                .error((error) => {
    ...
    <button
        ...
        <span x-show="groupUnread > 0 && supportUnread === 0 && !isOpen && !groupChatOpen" x-text="groupUnread > 99 ? '99+' : groupUnread" class="chat-unread-badge"></span>
        {{-- Badge support unread (hỗ trợ khách hàng - chỉ hiện khi chatbox đang đóng) --}}
        <span x-show="supportUnread > 0 && !isOpen" x-text="supportUnread > 99 ? '99+' : supportUnread" class="chat-unread-badge"></span>
